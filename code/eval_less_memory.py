...
### My libs
from tianchi_dataset import DAVIS_MO_Test
...
# palette = Image.open(DATA_ROOT + '/Annotations/625644/00000.png').getpalette()
...
Testset = DAVIS_MO_Test(DATA_ROOT, imset='{}.txt'.format(SET), single_object=(YEAR == 16))
...
pth_path = '../user_data/model_data/STM_weights.pth'
...
for seq, V in enumerate(Testloader):
    ...
    test_path = os.path.join('../user_data/tmp_data/', seq_name)
    ...
    palette = Image.open(DATA_ROOT + '/Annotations/' + seq_name + '/00000.png').getpalette()
    # if num_frames >= 120:
    #     mem_every = int(num_frames // 6)
    # elif num_frames >= 100:
    #     mem_every = int(num_frames // 8)
    # elif num_frames >= 80:
    #     mem_every = int(num_frames // 10)
    # else:
    #     mem_every = int(num_frames // 12)
    # mem_every = max(int(num_frames // 20), 5)
    mem_every = int(num_frames // 3)
    pred, Es = Run_video(Fs, Ms, num_frames, num_objects, Mem_every=mem_every, Mem_number=None)
